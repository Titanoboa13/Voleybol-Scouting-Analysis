...
- **Player Profile tab** (tab 5): shows an initials avatar (coloured by position via `POSITION_COLORS` — a constant dict keyed by position number, reusable for future charts/badges), player name with a coloured position badge, age computed from `date_of_birth` (parsed once in `load_data()` via `pd.to_datetime(format='%d/%m/%Y', errors='coerce')` into a `dob` column; `_compute_age()` converts it to an integer at render time so the cached frame never goes stale), and six `st.metric` stat tiles. Detailed AI scouting profile via `generate_player_profile(player_name, language)` (`@st.cache_data`, same Gemini + graceful-degradation pattern, TR/EN toggle, 3-paragraph structure: Strengths / Weaknesses / Overall). Photo upload is deferred to a future phase. **Country code is intentionally not displayed** — the `country` column contains opaque numeric codes with no mapping stored in the data (known examples: 23 = Russia, 30/31 = Brazil). The column is kept in the dataframe for future use but hidden from the UI to avoid confusing users. The full dataset is outdated/incomplete and is slated for a full replacement in a future release.
